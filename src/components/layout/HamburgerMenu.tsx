...
export default function HamburgerMenu({
  ...
  return (
    <div className={"fixed left-0 top-0 bottom-0"}>
      <button
        className={"h-[52px] p-4 cursor-pointer z-50"}
        onClick={() => {
          openWithAnimation();
        }}
      >
      ...
      <div
        className={
          "fixed left-0 top-0 bottom-0 w-[400px] shadow-2xl bg-white" +
          ` translate-x-[-100%] duration-${DURATION} transition-transform`
        }
        ref={contentRef}
      >
        <div className={"flex"}>
          <div
            className={"fixed h-[48px] p-1 py-3 cursor-pointer"}
            onClick={() => {
              closeWithAnimation();
            }}
          >
