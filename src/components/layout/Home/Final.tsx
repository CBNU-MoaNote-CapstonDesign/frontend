...
function SeeMoreButton({ text, onClick }: { text: string, onClick: () => void }) {
    return (
        <button
            className="mt-6 bg-[#186370] hover:bg-[#1e7a8a] text-white px-10 py-3 text-2xl font-semibold rounded-full shadow-md transition"
            onClick={onClick}>
            {text}
        </button>
    );
...
export default function Final() {
    ...
    return (
        <div className="w-full max-w-screen-xl px-4 mt-32 flex flex-col lg:flex-row items-center justify-between gap-10">
            ...
            <img src="/moanote_logo/logo2.png" className="w-64 md:w-80 object-contain drop-shadow-lg" alt="logo2" />
        </div>
    );
}