...
const getDiff = (previousString: string, newString: string): {removeFrom: number, removeLength: number, insertedContent?: string} => {
  ...
  removeEnd = removeFrom;
  for (let i = 0; previousString.length - i - 1 > removeFrom && newString.length - i - 1 >= 0; i++) {
    if (previousString[previousString.length - i - 1] !== newString[newString.length - i - 1]) {
      removeEnd = previousString.length - i - 1;
      break;
    }
  }

  const removeLength = removeEnd - removeFrom + 1;

  const insertedContent = removeFrom >= newString.length ?
      undefined : newString.slice(removeFrom, newString.length - (previousString.length - removeEnd - 1));
  return {
      removeFrom,
      removeLength,
      insertedContent
