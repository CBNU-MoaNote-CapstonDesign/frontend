 ...
 * @constructor
 */
const useDiagramSync = (user:User, uuid:string, update:(content: string)=>void) => {
  ...
  const publish = (content: string) => {
    if (clientRef.current && clientRef.current.connected) {
      ...
      clientRef.current.publish({
        destination: `/app/docs/diagram/edit/${uuid}`,
    ...
    const client = new Client({
      ...
      onConnect: () => {
        if (process.env.DEBUG==='true')
          toast("연결시도 끝");
        client.subscribe(`/app/docs/diagram/participate/${docsSubWith.docId}`, message => {
          try {
